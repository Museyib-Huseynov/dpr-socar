...
export async function GET(request, { params }) {
  ...
  let query = `
  SELECT
    TO_CHAR(report_date, 'YYYY-MM-DD') AS "Tarix",
    field AS "Yataq",
    platform AS "Özül / Mədən",
    well AS "Quyu",
    ${downloaded_data.includes(1) ? `well_stock_category AS "Quyu fondu",` : ``}
    ${
    ...
    ${
      downloaded_data.includes(10) ? `well_uptime_hours AS "İşləmə saatı",` : ``
    }
    ...
    ${downloaded_data.includes(42) ? `pcp_rpm AS "Dövrlər sayı (Vintli)",` : ``}
    ${
    ...
    ${downloaded_data.includes(45) ? `production_skin AS "İtki təbəqəsi",` : ``}
